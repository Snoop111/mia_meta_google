...
async def comprehensive_insights(request: ComprehensiveInsightsRequest):
    ...
    try:
        ...
        # Build a per-user integrator configured with this user's stored connectors
        user_integrator = credential_manager.build_integrator_for_user(user_id)
            ...
            try:
                ...
                if data_source == 'ga4':
                    platform_data[platform] = await _fetch_ga4_data(
                        user_integrator, user_id, start_date, end_date, selection.property_id
                    )
                elif data_source == 'google_ads':
                    platform_data[platform] = await _fetch_google_ads_data(
                        user_integrator, user_id, start_date, end_date, selection.account_id
                    )
                ...
                if platform_data[platform] is not None and hasattr(platform_data[platform], 'empty') and not platform_data[platform].empty:
        ...
        combined_insights = {}
        available_platforms = [p for p, data in platform_data.items() if data is not None and hasattr(data, 'empty') and not data.empty]
        ...
        response = {
            ...
            "data_availability": {
                platform: data is not None and hasattr(data, 'empty') and not data.empty 
                for platform, data in platform_data.items()
            }
        }
...
async def _fetch_ga4_data(integrator: 'data_integrator.DataIntegrator', user_id: str, start_date: str, end_date: str, property_id: str = None):
    """Fetch GA4 data with comprehensive metrics"""
    try:
        connector = None
        for name, conn in integrator.connectors.items():
            if name == 'ga4':
                connector = conn
                break
...
async def _fetch_google_ads_data(integrator: 'data_integrator.DataIntegrator', user_id: str, start_date: str, end_date: str, account_id: str = None):
    """Fetch Google Ads data with comprehensive metrics"""
    try:
        ad_sources = ['google_ads']
        connector_config = {}
        if account_id:
            connector_config['customer_id'] = account_id
            
        data = await integrator.fetch_specific_data(
            connector_names=ad_sources,
            start_date=start_date,
            end_date=end_date,
            connector_configs={'google_ads': connector_config} if account_id else None
        )
        return data
    except Exception as e:
...
async def _fetch_meta_ads_data(integrator: 'data_integrator.DataIntegrator', user_id: str, start_date: str, end_date: str, account_id: str = None):
    """Fetch Meta Ads data with comprehensive metrics"""
    try:
        ad_sources = ['meta_ads']
        connector_config = {}
        if account_id:
            connector_config['account_id'] = account_id
            
        data = await integrator.fetch_specific_data(
            connector_names=ad_sources,
            start_date=start_date,
            end_date=end_date,
            connector_configs={'meta_ads': connector_config} if account_id else None
        )
        return data
    except Exception as e:
...
async def _generate_ga4_insights(data):
    ...
    try:
        ...
        numeric_columns = ['sessions', 'newUsers', 'screenPageViews', 'userEngagementDuration', 
                          'keyEvents', 'totalRevenue', 'engagementRate']
        ...
        if 'sessions' in data.columns:
            insights["traffic_overview"] = {
                ...
                "total_users": int(data['newUsers'].sum()) if 'newUsers' in data.columns else 0,
                "total_page_views": int(data['screenPageViews'].sum()) if 'screenPageViews' in data.columns else 0,
                "avg_engagement_rate": float(data['engagementRate'].mean()) if 'engagementRate' in data.columns else 0.0,
                "total_key_events": int(data['keyEvents'].sum()) if 'keyEvents' in data.columns else 0,
                "total_revenue": float(data['totalRevenue'].sum()) if 'totalRevenue' in data.columns else 0.0
            }
        
        # Top channels
        if 'sessionDefaultChannelGrouping' in data.columns and 'sessions' in data.columns:
            try:
                ...
                insights["top_channels"] = channel_performance.to_dict('records')
            except Exception as e:
                logger.warning(f"Channel performance calculation failed: {e}")
                insights["top_channels"] = []
        
        # Geographic insights
        if 'country' in data.columns and 'sessions' in data.columns:
            try:
                geo_performance = data.groupby('country')['sessions'].sum().head(10)
                insights["geographic_performance"] = geo_performance.to_dict()
            except Exception as e:
                logger.warning(f"Geographic performance calculation failed: {e}")
                insights["geographic_performance"] = {}
        
        # Device insights
        if 'deviceCategory' in data.columns and 'sessions' in data.columns:
            try:
                device_performance = data.groupby('deviceCategory')['sessions'].sum()
                insights["device_breakdown"] = device_performance.to_dict()
            except Exception as e:
                logger.warning(f"Device performance calculation failed: {e}")
                insights["device_breakdown"] = {}
        
        # Source/Medium insights
        if 'sessionSourceMedium' in data.columns and 'sessions' in data.columns:
            try:
                source_performance = data.groupby('sessionSourceMedium')['sessions'].sum().head(10)
                insights["top_sources"] = source_performance.to_dict()
            except Exception as e:
                logger.warning(f"Source performance calculation failed: {e}")
                insights["top_sources"] = {}
        
        # Campaign insights
        if 'sessionCampaignName' in data.columns and 'sessions' in data.columns:
            try:
                # Filter out (not set) and null values
                campaign_data = data[data['sessionCampaignName'].notna() & 
                               (data['sessionCampaignName'] != '(not set)')]
                if hasattr(campaign_data, 'empty') and not campaign_data.empty:
                    campaign_performance = campaign_data.groupby('sessionCampaignName')['sessions'].sum().head(10)
                    insights["top_campaigns"] = campaign_performance.to_dict()
                else:
                    insights["top_campaigns"] = {}
            except Exception as e:
                logger.warning(f"Campaign performance calculation failed: {e}")
                insights["top_campaigns"] = {}
    
    except Exception as e:
        logger.error(f"GA4 insights generation failed: {e}")
...
async def _generate_combined_insights(platform_data: dict, available_platforms: list, min_spend_threshold: float, budget_increase_limit: float):
    ...
    try:
        ...
        if (ga4_data is not None and hasattr(ga4_data, 'empty') and not ga4_data.empty and 
            ads_data is not None and hasattr(ads_data, 'empty') and not ads_data.empty):
